...
1. In Visual Studio Code, navigate to explorer and your *WORKSHOP_PROJECT* section where you previously made `iis_advanced`.
...
2. Within *iis_simple* create new folders as follows:
...
1. Make a backup copy of `site.yml`, then create a new `site.yml`.
...
2. Update site.yml to look like to only call your role. It should look like below:
...
3. Add a default variable to your role. Edit the `roles\iis_simple\defaults\main.yml` as follows:
...
4. Add some role-specific variables to your role in `roles\iis_simple\vars\main.yml`.
...
5. Create your role handler in `roles\iis_simple\handlers\main.yml`.
...
6. Add tasks to your role in `roles\iis_simple\tasks\main.yml`.
...
----
7. Add your index.html template.
